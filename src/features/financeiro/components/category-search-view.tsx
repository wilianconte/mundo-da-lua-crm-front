...
import { getCategories, type CategoryFilterInput, type CategoryNode, type GetCategoriesVariables } from "../api/get-categories";
...
export function CategorySearchView() {
  ...
  const where = useMemo<CategoryFilterInput | null>(() => {
    const nextWhere: CategoryFilterInput = {};
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        ...
        const variables: GetCategoriesVariables = {
          where,
        ...
        const response = await getCategories(variables);
        if (!active) return;
        ...
        setTotalCount(0);
        setErrorMessage(
          error instanceof GraphQLRequestError ? error.message : "Nao foi possivel carregar categorias."
        );
      } finally {
    ...
    <div className="space-y-6">
      <FeatureViewHeader
        actions={
          <Button asChild className="min-w-40" leadingIcon={<Plus className="size-4" />}>
            <Link href="/financeiro/categorias/cadastro">Adicionar</Link>
          </Button>
        }
        backAriaLabel="Voltar para financeiro"
        backHref="/financeiro"
        description="Gerencie as categorias de transacoes do financeiro"
        title="Categorias"
      />
      ...
      <SearchResultsTable
        ...
        columns={[
          ...
          { label: "Data Criacao", sortKey: "createdAt" },
          { label: "Acoes" }
        ]}
        ...
        renderRow={(category) => (
          ...
          >
            ...
            <td className="px-4 py-3">{toDate(category.createdAt)}</td>
            <td className="px-4 py-3">
              <Button asChild size="sm" variant="outline">
                <Link href={`/financeiro/categorias/cadastro?mode=edit&id=${category.id}`}>Editar</Link>
              </Button>
            </td>
          </tr>
        )}
        ...
        rows={rows}
        sortBy={sortBy}
        totalText={`${totalCount} categoria(s) encontrada(s).`}
      />
    </div>
  );
}
