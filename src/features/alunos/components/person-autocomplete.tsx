...
export function PersonAutocomplete({
  ...
  }, [query, excludedPersonIds]);

  const helperText = useMemo(() => {
    if (value) {
      return `${value.fullName} · ${value.documentNumber} · ${value.phone}`;
    }

    return label ? `Selected ${label.toLowerCase()} will appear here.` : "";
  }, [label, value]);
          ...
          {isOpen ? (
            <div className="absolute z-20 mt-2 max-h-72 w-full overflow-auto rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-surface)] shadow-[var(--shadow-soft)]">
              {results.length ? (
                ...
                <div className="px-4 py-4 text-sm text-[var(--color-muted-foreground)]">No matches found.</div>
              )}
            </div>
          ) : null}
        </div>
